...
from keras.layers import Conv2D, MaxPooling2D, Input, GRU, Dense, Activation, Dropout, Reshape, Flatten, CuDNNGRU, BatchNormalization
...
def create_model(speaker_target_names_count, input_tensors_shape, dropout_rate):
    ...
    # Shape is now 8 x 4 x 64  where the first dimension in the time dimension.
    spec = Reshape((8, -1))(spec)
    ...
    spec = Bidirectional(CuDNNGRU(32, return_sequences=True), merge_mode='concat')(spec)
    spec = Dropout(dropout_rate)(spec)
    spec = Bidirectional(CuDNNGRU(32, return_sequences=True), merge_mode='concat')(spec)
    """
    spec = Dropout(dropout_rate)(spec)
